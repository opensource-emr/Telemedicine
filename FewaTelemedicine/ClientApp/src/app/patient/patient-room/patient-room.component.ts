...
export class PatientRoomComponent implements AfterViewInit {
  ...
  constructor(private notificationService: NotificationService,
    ...
    if (!this.isMobile) {
      ...
      this.notificationService.EventGetAllProviders.subscribe(_providers => {
        this.providerObj = _providers.find(a => a.url == this.global.currentProvider && a.practice == this.global.currentPractice);;
        if (this.providerObj) {
      ...
      if (this.global.providerObj)
        if (this.global.providerObj.image) {
          this.retrievedImage = 'data:image/png;base64,' + this.global.providerObj.image;
        }
      // gets doctor list
      // this.notificationService.LoadActiveDoctors();
    }
